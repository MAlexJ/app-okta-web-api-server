...
import com.malexj.models.ErrorMessage;
...
public record GlobalErrorHandler(ObjectMapper mapper) implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(final ServerWebExchange exchange, final Throwable error) {
        ...
        if (error instanceof ResponseStatusException statusError) {
            ...
            if (status.equals(HttpStatus.NOT_FOUND)) {
                final var body = this.makeBodyBytes("Not Found").map(bufferFactory::wrap);
        ...
        return response.writeWith(this.makeBodyBytes(error.getMessage()).map(bufferFactory::wrap));
    }
    ...
    public Mono<Void> handleAuthenticationError(final ServerWebExchange exchange, final AuthenticationException error) {
        ...
        final var body = this.makeBodyBytes(message).map(response.bufferFactory()::wrap);
    ...
    private Mono<byte[]> makeBodyBytes(final String message) {
        return Mono.create(sink -> Maybe.just(message).map(ErrorMessage::from).resolve(mapper::writeValueAsBytes).doOnSuccess(sink::success).doOnError(sink::error));
    }
}
